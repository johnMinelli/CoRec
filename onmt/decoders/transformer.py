...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, inputs, memory_bank, src_pad_mask, tgt_pad_mask,
        ...
        mid, context_heads_attn, attn = self.context_attn(memory_bank, memory_bank, query_norm,
                                      mask=src_pad_mask,
                                      layer_cache=layer_cache,
                                      type="context")
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, num_layers, d_model, heads, d_ff, attn_type,
        ...
        self.transformer_layers = nn.ModuleList(
            [TransformerDecoderLayer(d_model, heads, d_ff, dropout, self_attn_type=self_attn_type) for _ in range(num_layers)])
    ...
    def forward(self, tgt, memory_bank, memory_lengths=None, step=None, cache=None, **kwargs):
        """
        ...
        src_pad_mask = src_words.data.eq(padding_idx).unsqueeze(1).expand(src_batch, tgt_len, src_len)  # [B, 1, T_src]
        tgt_pad_mask = tgt_words.data.eq(padding_idx).unsqueeze(1).expand(tgt_batch, tgt_len, tgt_len)  # [B, 1, T_tgt]
